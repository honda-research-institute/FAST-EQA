...
class VLM:
    def __init__(self, cfg):
        self.temp_prismatic = cfg.temp_prismatic
        self.temp_gpt = cfg.temp_gpt
        self.seed = cfg.seed
        ...
        if cfg.model_name == 'prismatic':
            ...
            logging.info(f"Loaded Prismatic VLM in {time.time() - start_time:.3f}s")
            logging.info(f"Using GPU: {next(self.model.parameters()).device}")
    ...
    def generate(self, prompt, images, T=0.4, max_tokens=512):
        if self.model_name == 'prismatic':
            ...
            generated_text = self.model.generate(
                ...
                prompt_text,
                do_sample=False,
                temperature=self.temp_prismatic,
            ...
            response = self.client.chat.completions.create(
                ...
                temperature=self.temp_gpt,
                seed=self.seed,
                logprobs=True
            )

            ret = response.choices[0].message.content
            prob = response.choices[0].logprobs
            # print("gpt ret: ", ret, " prob: ", prob)
            return ret
    ...
    def query_gpt(self, prompt, images, multichoice=False, T=0.4, max_tokens=512):
        ...
        while tries > 0:
            ...
            ret = response.choices[0].message.content
            prob = response.choices[0].logprobs
            # logging.info(f"GPT-4o Output: {ret}, prob: {prob}")
    ...
    def parse_goals(self, output):
        """
        Parses the output from the LLM to extract goals in JSON format.

        Uses for combined prompting with LLMs to extract goals for exploration tasks.
        """
    ...
    def extract_goals_and_rooms(self, question):
        """
            This is an alternate version of extract_goals() + get_rooms_needed() that gets rooms and visual goals with one GPT call
            ...
            goals = vlm.extract_goals_and_rooms(question)  --> This function
    ...
    '''
        ...
        while tries > 0:
            ...
            # logging.info(f"gpt ret: {ret}, prob: {prob}")
    ...
    def extract_goals(self, question):
        prompt = '''You are an agent tasked with exploring an environment to answer a question.
    Given the question, please plan the steps you need to take or things you need to observe to answer the question.
    Think about the rooms to explore and object to observe, including the anchor object and final target object.
    Please give the steps in terms of a list of dictionaries, one for each subtask: [{"room": "room_name", "anchor": "object_name", "target": "object_name"}].
    ONLY output pure JSON and nothing else. If no target object is mentioned, do not output a target.

    For example: 
    Question: What color is the kettle on the counter in the kitchen?
    Output:
    [{"room":"kitchen", "anchor":"counter", "target":"kettle"}]

    Question: Where did I leave my blue water bottle? I can't find it.
    Output:
    [{"target":"blue water bottle"}]

    Question: What is on the bedroom nighstand?
    Output:
    [{"room":"bedroom", "anchor":"nightstand"}]

    Question: Is the door the same material as the ceiling?
    Output:
    [{"target":"door"}, {"target":"ceiling"}]

    Question: Is the living room coffee table the same color as the dining table?
    Output:
    [{"room":"living room", "target":"coffee table"}, {"room":"dining room", "target":"dining table"}]
    ...
    '''
        ...
        while tries > 0:
            ...
            ret = response.choices[0].message.content
            parsed  = self.parse_plan_json(ret)
    ...
    def get_rooms_needed(self, question):

        client = Mistral(api_key=self.mistral_key)

        # prompt
        prompt = f'''You are a robot exploring a scene to answer the question: {question}.
    Which rooms do you think you need to explore to answer the question? Answer only with a list of room names from this list: [hallway, dining room, living room, kitchen, bedroom, bathroom, entryway, storage room, gym, home office, laundry room, garage, porch]
    You may list multiple rooms if unsure. If a question does not mention a room, guess what rooms may be likely from the list. 
    DO NOT answer with any other room names!

    Example: 
    Question: How many plants are near the kitchen sink?
    Output: ['kitchen']

    Question: Is the bed in the bedroom bigger than the couch in the living room?
    Output: ['bedroom', 'living room']

    Question: Is the window open in the bed room with red walls?
    Output: ['bedroom']

    Question: Where did I leave my laptop?
    Output: ['home office', 'living room', 'bedroom']
    '''
        tries = 3
        while tries > 0:
            try:
                ...
                logging.info(f"Mistral Output: {res}")
