...
### NOTE: changed to load from local llama model
...
class HFCausalLLMBackbone(LLMBackbone, ABC):
    def __init__(
        ...
        # [Contract] `inference_mode` means we're loading from a pretrained checkpoint; no need to load base weights!
            ...
            llm_config = AutoConfig.from_pretrained('/data/01/public_models/llama2/llama-2-7b-hf') #(hf_hub_path, token=hf_token)
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('/data/01/public_models/llama2/llama-2-7b-hf') #llama3/Meta-Llama-3-8B')
        self.tokenizer.add_bos_token = True # PATCH
